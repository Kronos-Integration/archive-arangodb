...
module.exports = {
	...
	"initialize": function (conf) {
		if (!conf) {
			...
			_.defaultsDeep(conf, defaultConfiguration);
		}
	...
	"save": function (meta, data) {
		...
		if (data instanceof stream.Readable) {
			// first convert the stream to a blob
			return _getBlobFromStream(data).then(function (blob) {
				return _saveSub(blob, timeStamp, meta);
			});
		} else {
			return _saveSub(data, timeStamp, meta);
		}
...
/**
 * If the object given is a readable stream, first convert it to a blob.
 * @param blob The blob to save
 * @param timeStamp The timeStamp getting the save request
 * @param meta The meta data hash
 * @return promise A promise with the blob object
 */
function _saveSub(blob, timeStamp, meta) {
	return _initCollectionCache(config.collections).then(function () {
...
/**
 * If the object given is a readable stream, first convert it to a blob.
 * @param stream The stream to be converted to a blob
 * @return promise A promise with the blob object
 */
function _getBlobFromStream(stream) {
	let blob = '';
