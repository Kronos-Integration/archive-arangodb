...
module.exports = {
	...
	"save": function (meta, blob) {
		...
		if (blob === undefined) {
			return new Promise(function (fulfill, reject) {
				reject("No blob data given to be saved");
			});
		}
		...
		if (meta === undefined) {
			return new Promise(function (fulfill, reject) {
				reject("No meta data given to be saved");
			});
		}
	...
	"get": function (id) {
		...
		if (id === undefined) {
			return new Promise(function (fulfill, reject) {
				reject("No id given to retrieve the data for.");
			});
		}
	...
	"delete": function (id) {
		...
		if (id === undefined) {
			return new Promise(function (fulfill, reject) {
				reject("No id given to remove the data for.");
			});
		}
...
function _getBlobData(id) {
	return collectionCache[edgeName].outEdges(id).then(function (edgeDoc) {
		...
		return collectionCache[blobName].document(edgeDoc[0]._to).then(function (blobDoc) {
			// we got the blob doc
			return new Promise(function (fulfill, reject) {
				fulfill(new Buffer(blobDoc.data, 'base64').toString('ascii'));
			});
		});
	});
}
		...
		if (cursor.hasNext()) {
			...
			return collectionCache[blobName].save({
				...
				if (err.errorNum == 1210) {
					...
					return new Promise(function (fulfill, reject) {
						fulfill(config.collections.collectionNameBlob + "/" + blobHash);
					});
				} else {
					return new Promise(function (fulfill, reject) {
						reject(err);
...
function _getCollection(collectionName, collectionCache, cacheName) {
	if (collectionCache[cacheName]) {
		...
		return new Promise(function (fulfill, reject) {
			fulfill(collectionCache[cacheName]);
		});
	} else {
